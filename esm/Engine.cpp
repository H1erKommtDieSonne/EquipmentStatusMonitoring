...
std::vector<DeviceRow> Engine::list_devices() const {
    // ¬озьмЄм все адреса, а затем подт€нем пол€ (чтобы не писать ещЄ один репозиторий):
...
std::int64_t Engine::breakdown(std::uint32_t address, const std::string& fault) {
    ...
    // 1) —оздать/обновить запись об устройстве как неисправной (если нет Ч вставим заглушку).
    storage_.exec("BEGIN IMMEDIATE;");
    {
        const char* UPSERT =
            "INSERT INTO devices(address,name,priority,is_faulty,is_reserve) "
            "VALUES(?1,'Unknown',2,1,0) "  // приоритет по умолчанию Low(=1) или High(=2) Ч на твоЄ усмотрение
...
void Engine::finish_repair(std::int64_t job_id, std::uint64_t uptime_after_sec) {
    ...
    // 3) отметим device как исправный (is_faulty=0) + upsert HealthyDevice дл€ согласованности
    {
        const char* DEV =
    ...
    // Ќа уровне домена можно обновить Ђнаработкуї (это уже другой слой Ч здесь просто отметили факт ремонта).
